...
int main(int argc, char** argv)
{
    ...
    if (sendAppCmd)
    {
	if (appCmd.contains("AD-")) {
		if (verbose) cout << "[AD-] Sending AD- bootloading commands..." << endl;
		...
		// If false on starboot, then return with error:
	...
	} else {	// otherwise, the sendout the actual appCmd:
		if (verbose)
	...
	{
		if (verbose) cout << "EEPROM:  FAILED" << endl;
		success = false;
        	//return 1;
	} else {
		if (verbose) cout << "EEPROM:  SUCCESS" << endl;
	}
    }
	...
	{
		if (verbose) cout << "EEPROM READ:  FAILED" << endl;
		success = false;
        	//return 1;
	} else {
		if (verbose) cout << "EEPROM READ:  SUCCESS" << endl;
  ...
  if (appCmd.contains("AD-") || appCmd.contains("TEST-")) {
	...
	if (success) {
		if (doConnect) {	// do this only Connected to the bootloader:
			if (verbose) cout << "SUCCESS:  Run the firmware..." << endl;
			port->putChar('g');
			port->putChar('\n');
		}
		...
		cout << "SEND TEST COMMANDS..." << endl;
		Msleep(1000);
		// ALL OFF
		port->write("*ZALLPWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		// TURNING ON 1 by 1
		port->write("*ZN01PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN02PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN03PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN04PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN05PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN06PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN07PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN08PWR01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		// SOURCE SET
		port->write("*ZN01SRC01\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN02SRC02\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN03SRC03\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN04SRC04\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN05SRC05\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN06SRC06\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN07SRC07\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN08SRC08\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		// POWER OFF
		Msleep(1000);
		port->write("*ZN08PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN07PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN06PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN05PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN04PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN03PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN02PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		port->write("*ZN01PWR00\r");
		port->flushOutBuffer();
		Msleep(cmdWaitTime);
		// ALL ON
